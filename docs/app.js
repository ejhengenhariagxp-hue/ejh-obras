...
G.migrarFotosStorage = async () => {
  ...
  try {
    ...
    // Remove a entrada antiga antes de gravar a menor — evita QuotaExceededError
    // quando o storage está cheio e o setItem tentaria alocar espaço adicional
    // antes de liberar o antigo.
    try { localStorage.removeItem('ejh_obras_v4'); } catch (_) {}
    _lastHash = ''; // força saveStateLocal mesmo que o hash não tenha mudado
    saveStateLocal();
...
function saveStateLocal() {
  ...
  try {
    ...
    if (isQuota) {
      // Remove a entrada antiga primeiro para liberar espaço, depois regrava.
      // Sem isso o setItem precisa alocar espaço antes de liberá-lo e falha
      // mesmo que a nova versão fosse menor que a antiga.
      try {
