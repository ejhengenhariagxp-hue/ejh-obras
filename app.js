...
import { novaComposicao, addInsumoComp, renderInsumosComp, calcTotalComp, salvarComposicao,
         delComposicao, renderComposicoes, filtrarComposicoes, popularSelectComposicoes,
         preencherDadosComposicao, calcTotalComposicaoSel, inserirComposicaoNoOrcamento,
         editarComposicao } from './modules/composicoes.js';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  G.salvarComposicao            = () => {
    ...
    if(editId) {
      // Modo edição — remove a antiga e salva nova com mesmo ID
      const idx = state.composicoes.findIndex(c=>c.id===editId);
      if(idx>=0) state.composicoes.splice(idx,1);
      window._editandoCompId = null;
    }
    if(salvarComposicao(state)) renderAtiva();
  };
  ...
  G.abrirInserirComposicao      = id => {
    ...
    if(sel && id) { sel.value=id; preencherDadosComposicao(state); }
  ...
  // ── Import / Planilha handlers ──────────────────────────────────────
  G.importManual      = () => { if(typeof importManual==='function') importManual(); };
  G.importExcel       = (inp) => { if(typeof importExcel==='function') importExcel(inp); };
  G.importPDF         = (inp) => { if(typeof importPDF==='function') importPDF(inp); };
  G.importCSV         = (inp) => { if(typeof importCSV==='function') importCSV(inp); };
  G.confirmImport     = () => { if(typeof confirmImport==='function') confirmImport(state); };
  G.cancelImport      = () => { if(typeof cancelImport==='function') cancelImport(); };
  G.addImportRow      = () => { if(typeof addImportRow==='function') addImportRow(); };
  G.applyMapping      = () => { if(typeof applyMapping==='function') applyMapping(state); };
  G.parseJsonConfig   = () => { if(typeof parseJsonConfig==='function') parseJsonConfig(); };
  G.showJsonPaste     = () => { if(typeof showJsonPaste==='function') showJsonPaste(); };
  G.salvarConfigurarFb = () => { if(typeof salvarConfigurarFb==='function') salvarConfigurarFb(); };
  G.capAnalisarPreview = () => {};  // preview inline, não crítico
  G.capProcessarArquivo = (inp) => { if(typeof capProcessarArquivo==='function') capProcessarArquivo(inp); };
  G.renderHistoricoCaptura = () => { if(typeof renderHistoricoCaptura==='function') renderHistoricoCaptura(state); };
});
